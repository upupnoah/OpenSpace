...
contract InscriptionERC20 is Initializable, IERC20, IERC20Errors {
    // 单次创建量
    uint256 private s_perMint;
    // 限制的总供应量上线
    uint256 private s_limit_totalSupply;
    ...
    function initialize(address owner_, string memory symbol_, uint256 totalSupply_, uint256 perMint_)
        public
        initializer
    {
        s_perMint = perMint_;
        s_limit_totalSupply = totalSupply_;
        _name = "InscriptionERC20";
        _symbol = symbol_;
        s_owner = owner_;
        // 将所有 token 发给 owner
    ...
    function mint(address to) public {
        // 检查供应量是否超过上限
        require(totalSupply() + s_perMint <= s_limit_totalSupply, "totalSupply exceed limit");
        _mint(to, s_perMint);
    }
